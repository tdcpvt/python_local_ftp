...
def load_db():
    if not os.path.exists(DB_FILE):
        default_db = {
            ...
            "settings": {
                "allowed_extensions": list(upload_handler.DEFAULT_ALLOWED_EXTENSIONS)
            },
            "file_registry": []
        }
    ...
    with open(DB_FILE, 'r') as f:
        try:
            ...
            if "settings" not in data: data["settings"] = {"allowed_extensions": list(upload_handler.DEFAULT_ALLOWED_EXTENSIONS)}
            return data
        except Exception:
            return {"users": {"admin": {"password": "password123", "role": "admin", "can_delete": True}}, "settings": {"allowed_extensions": list(upload_handler.DEFAULT_ALLOWED_EXTENSIONS)}, "file_registry": []}
...
# --- REWORKED SECURE ROUTE ROUTING VIA THE HANDLER ---
...
def upload_file(target_user, folder_type):
    ...
    # Pass data downstream to our separate file upload processor process module
    success, result = upload_handler.process_file_upload(
        ...
        remark=remark,
        base_storage_path=BASE_STORAGE,
        db_config=db
    )

    if not success:
        # Return error text clear layout signals to browser AJAX triggers
        return jsonify({"error": result}), 400

    # If successful, result contains clean metadata to log to the persistent database
